...
def distribution_false_discoveries_params(m=None, qm=None, beam_search_params=None, dataset=None, attributes=None, descriptives=None, 
                                          wcs_params=None, constraints=None):

    beam_search_params_temp = beam_search_params.copy()
    beam_search_params_temp['q'] = 1
    distribution = make_distribution_false_discoveries(m=m, qm=qm, 
...
def make_distribution_false_discoveries(m=None, qm=None, 
    ...
    qm_values = Parallel(n_jobs=-2)(delayed(make_false_discovery)(i, qm, dataset, attributes, descriptives, 
                                                                  beam_search_params_temp, wcs_params,
                                                                  constraints) for i in inputs)

    return qm_values

def make_false_discovery(i=None, qm=None, 
                         dataset=None, attributes=None, descriptives=None, 
                         beam_search_params_temp=None, wcs_params=None, constraints=None):
    ...
    result_emm, general_params, considered_subgroups = bs.beam_search(dataset=shuffled_dataset, attributes=attributes, descriptives=descriptives, 
                                                                      qm=qm, beam_search_params=beam_search_params_temp, wcs_params=wcs_params,
                                                                      constraints=constraints)

    # save qm of the first subgroup
    qm_value = result_emm.loc[result_emm.sg == 0, qm].values[1] # 1 is qualities
